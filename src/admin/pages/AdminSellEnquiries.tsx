...
import { ShoppingBag, Search, Phone, MapPin, Calendar } from 'lucide-react';
import { supabase } from '../../lib/supabase';

interface SellEnquiry {
  id: string;
  name: string;
  phone: string;
  location: string;
...
export default function AdminSellEnquiries() {
  const [items, setItems] = useState<SellEnquiry[]>([]);
  ...
  const filtered = items.filter(i => {
    ...
    return !q || i.name.toLowerCase().includes(q) || i.phone.includes(q) || i.location.toLowerCase().includes(q);
  });

  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[#0a2240]">Sell Property Enquiries</h1>
        <p className="text-gray-500 text-sm mt-0.5">{items.length} people want to sell their property</p>
      </div>
                ...
                {filtered.map(e => (
                  <tr key={e.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3 hidden sm:table-cell">
                      <a href={`tel:${e.phone}`} className="flex items-center gap-1.5 text-gray-600 hover:text-[#c9a84c] text-sm">
                        <Phone className="w-3.5 h-3.5" /> {e.phone}
                      </a>
                    </td>
                    <td className="px-4 py-3 hidden md:table-cell">
                      <div className="flex items-center gap-1.5 text-gray-600 text-sm">
                        <MapPin className="w-3.5 h-3.5 text-[#c9a84c]" /> {e.location}
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="flex items-center justify-between px-6 py-4 border-b">
              <h2 className="font-bold text-[#0a2240]">Sell Enquiry Details</h2>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <p className="text-xs text-gray-500 mb-0.5">Property Location</p>
                  <p className="font-medium text-gray-700">{selected.location}</p>
                </div>
              </div>
              {selected.message && (
                <div>
                  <p className="text-xs text-gray-500 mb-1.5">Additional Details</p>
                  <p className="text-gray-700 text-sm bg-gray-50 rounded-xl p-4 leading-relaxed">{selected.message}</p>
                </div>
              )}
            </div>
            <div className="px-6 pb-6">
              <a href={`tel:${selected.phone}`} className="flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white py-3 rounded-xl text-sm font-semibold w-full">
                <Phone className="w-4 h-4" /> Call {selected.name}
              </a>
            </div>
          </div>
        </div>
